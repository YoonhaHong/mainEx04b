...
ifeq ($(ROOT_USE),true)
	$(CXX) $< main92.so -o $@ -w -I$(ROOT_INCLUDE) $(CXX_COMMON)\
	 $(shell root-config --cflags) -Wl,-rpath,./\
...
mainEx00 mainEx01 mainEx02 mainEx03 mainEx04 mainEx04a mainEx04b mainEx05: $$@.cc $(PREFIX_LIB)/libpythia8.a
ifeq ($(FASTJET3_USE)$(ROOT_USE),truetrue)
	$(CXX) $< -o $@ $(FASTJET3_INCLUDE) $(shell root-config --cflags) $(CXX_COMMON)\
	 $(FASTJET3_LIB)\
